...
threshold = [32, 128]
max_label_len = np.load(STATIC_FOLDER +'/npy-data/max_label_len.npy')
...
model.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, 
                optimizer=optimizer)
model.load_weights(STATIC_FOLDER +'/model/best_model.h5')
def resize_(img): 
    w, h = img.shape
    if (32/w < 128/h):
        scale_percent = 32/w
...
def upload_file():
    ...
    if request.method == "GET":
        ...
        # custom filename
        filename = secure_filename("img_"+timestr+".png")
        upload_image_path = os.path.join(UPLOAD_FOLDER, filename)
        print(upload_image_path)
        file.save(upload_image_path)

        img = cv2.imread(upload_image_path, cv2.IMREAD_GRAYSCALE)
    ...
    return render_template(
        "predict.html", image_file_name=filename, pred=pred_str
    )
